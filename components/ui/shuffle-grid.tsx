...
export const ShuffleHero = ({
  ...
  return (
    <section className="w-full px-8 py-12 grid grid-cols-1 md:grid-cols-2 items-center gap-8 max-w-6xl mx-auto">
      <div>
        <span className="block mb-4 text-xs md:text-sm text-primary font-medium">{subtitle}</span>
        <h3 className="text-4xl md:text-6xl font-semibold text-foreground">{title}</h3>
        <p className="text-base md:text-lg text-muted-foreground my-4 md:my-6">{description}</p>
        <button
          className={cn(
...
const shuffle = (array: Array<{ id: number; src: string }>) => {
...
const generateSquares = (images: Array<{ id: number; src: string }>) => {
  const squareData = images.length > 0 ? images : defaultSquareData
...
const ShuffleGrid = ({ images = [] }: { images?: Array<{ id: number; src: string }> }) => {
